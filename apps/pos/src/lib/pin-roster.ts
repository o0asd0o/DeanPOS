// The hash-sync roster (issue 10, record 057 Q3) — the only other key in
// `apps/pos`'s localStorage besides device-token.ts's two. Replaced whole,
// never merged: a successful `terminal.pinSync` pull overwrites this
// atomically, so deactivating/reactivating a User just works on next sync.
