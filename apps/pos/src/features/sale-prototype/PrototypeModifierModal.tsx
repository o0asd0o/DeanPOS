...
import type { SaleAddOn, SaleMenuItem, SaleModifierGroup, SaleVariant } from "@/features/sale/types.ts";
import { formatDelta, formatPeso } from "./helpers.ts";
...
export function PrototypeModifierModal({
  item,
  variant,
  initial,
  onOpenChange,
  ...
  const toggleModifier = (group: SaleModifierGroup, id: string) => {
    setModifierIds((current) => {
      ...
      if (group.selectionRule === "many") {
        ...
        const chosen = current.filter((value) => group.modifiers.some((m) => m.id === value)).length;
        if (group.maximum !== null && chosen >= group.maximum) return current;
        return [...current, id];
      }
      if (group.selectionRule === "optional-one" && current.includes(id)) return withoutGroup;
  ...
  const changeAddOn = (addOn: SaleAddOn, amount: number) => {
    setAddOnIds((current) => {
      ...
      if (!canAddOn(addOn, current)) return current;
        ...
        <div className="flex flex-col gap-6">
          ...
          <fieldset className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-6">
              ...
              <span aria-label="Quantity" className="w-8 text-center text-xl font-semibold">
                {quantity}
              </span>
              <Button
                type="button"
