...
import { handler as resetPinHandler } from "backend/src/user/handlers/reset-pin.ts";
import { handler as resetUserPasswordHandler } from "backend/src/user/handlers/reset-user-password.ts";
import { handler as setPinHandler } from "backend/src/user/handlers/set-pin.ts";
...
export const userSetPinRoute = os.user.setPin.handler(({ context, input }) =>
  setPinHandler({ ctx: context, input }),
);
export const userResetPinRoute = os.user.resetPin.handler(({ context, input }) =>
  resetPinHandler({ ctx: context, input }),
);
