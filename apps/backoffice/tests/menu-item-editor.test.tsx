...
beforeAll(async () => {
  await ownerDb.insertInto("Tenant").values({ id: tenantId, name: "Menu Item Editor Tenant" }).execute();
  await ownerDb
    .insertInto("User")
    .values({
      id: adminId,
...
describe("one money formatter on catalog surfaces", () => {
  it("grep-proves catalog features use formatCentavos only", () => {
    ...
    expect(readFileSync(join(root, "helpers.ts"), "utf8")).toMatch(/export function formatCentavos/);
  });
});
...
describe("the MenuItem editor route", () => {
  ...
  it("loads the full-page editor, validates price bounds, creates a variant, and passes axe", { timeout: 20_000 }, async () => {
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: "New variant" })).toBeTruthy());
    ...
    fireEvent.click(within(screen.getByRole("heading", { name: "New variant" }).closest("form")!).getByRole("button", { name: /^Save$/ }));
    await waitFor(() =>
    ...
    await waitFor(() => expect(screen.queryByRole("heading", { name: "New variant" })).toBeNull());
