...
import { findCurrentActiveDiscountByName } from "../db-operations/queries/find-current-active-discount-by-name.query.ts";
...
export const handler: Handler<Input, ReturnType<typeof toDiscountOutput> | null> = async ({
  ...
  try {
    const row = await withTenantScope(ctx.db, ctx.principal.tenantId, async (db) => {
      ...
      const duplicate = await findCurrentActiveDiscountByName(
        db,
        current.name,
        current.discount_id,
      );
