...
- `employees-1440` / `stores-1440` — the list-screen pattern. Page header (title, one
  ...
  Nothing lives between the header and the Card.
- `employee-editor-1440` — the editor pattern. A detached right-hand sheet, not a route
  and not a page: fixed header, one scrolling body, fixed action row with Cancel left and
  the commit right. The Store editor is this same sheet with different fields.
- `deactivate-dialog-1440` — the confirmation pattern for a destructive, reversible action.
  Reactivation is never confirmed.
- `settings-dialog-1440` — a dialog rather than a screen, because there is exactly one row
  per Tenant and nothing to list. It opens from the account menu; the sidebar has no entry.
