...
and an optional maximum quantity, and is linked to the Variants it may accompany — so a cashier is
...
**Linking runs from the Variant side, not from a list of every Variant in the tenant.**
...
- [ ] An `AddOn` table with `tenant_id`, RLS enabled and forced in the creating migration,
      `@@unique([tenantId, id])`, and a link table to `Variant` with composite FKs on both sides.
- [ ] An Add-on carries a `Delta` with an explicit stored `kind`, reusing issue 03's column shape
      and its bounds — no second Delta implementation, proven by a grep.
- [ ] An optional maximum quantity; a maximum of `0` is refused by a database `CHECK`, matching
      how issue 03 refused `many` with maximum `0`.
- [ ] Add-ons archive, never delete. Archiving one removes it from every Variant it was linked to
      and those Variants stay sellable — archive cascade row 6.
- [ ] Linking happens from the Variant editor; the Add-ons list renders `LINKED VARIANTS` as
...
- [ ] The linked count excludes links to archived Variants, same rule as issue 04
      (`## Scenarios` row 26).
- [ ] The negative-effective-price guard from issue 04 covers Add-on linking and Add-on Delta
      edits — the same function, two more callers, not a second implementation.
- [ ] An Add-on with zero links is visibly flagged in the list as offered nowhere.
- [ ] The read model carries add-on links per Variant with their Delta and maximum.
- [ ] A quantity above an Add-on's maximum is **rejected server-side**.
