...
| Money | PHP, integer centavos, round half-up once at OrderLine total. A price is always what the customer pays — VAT is never added |
...
| 5 | `offline-sync` | Service worker + app shell precache, local catalog cache, IndexedDB Outbox, client-UUID stamping, idempotent replay endpoint, reconnect/backoff, duplicate + revoked-device handling, offline-visible sync status | `api` + `pos` | 1, 2, 3, 4 |
...
| 9 | `hardening` | Written threat model. Wrong-tenant authorisation tests against every reachable object. Device revocation enforced on replay. PIN throttling. Rate limits. CORS allowlist + origin-isolation tests. Secret handling + rotation. Dependency/advisory policy. Tenant export + purge path | all | 2, 4, 5 |
